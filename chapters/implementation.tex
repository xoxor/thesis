...
\commentOC{Not sure that these explanations are useful (too summarized for someone who is not familiar with these concepts already and too basics for the developers out there). But ok, it doesn’t hurt also and you never know, someone could get something out of it.}
...
Classes may inherit from other classes their actions and attributes. They can and \commentOC{Either there is one word too much here, or if you mean it, the sentence doesn’t seem grammatical to me.} implement abstract methods and add new functionality. Associations between classes represent these behaviors.
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
\commentOC{Minor, but, I’d use the same representation (that is, the Papyrus style) for the example class and for the real ones.}
...
The two basic elements are represented by the \textit{Alternative} and \textit{Voter} objects. A \textit{Preference} is a list of alternatives in which the order represents the preference order. Multiple alternatives can have the same rank but it is not possible to have the same alternative multiple times \commentOC{You mean that an alternative can’t have two ranks, or, be associated to different ranks? Or you could simply describe what it represents (a strict order or linear order depending on the term used elsewhere in this manuscript)}. Methods allow, for example, to know the alternative in a given rank or, conversely, to know the rank of a given alternative.
...
\commentOC{This ugly spacing is, I suspect, the price for the “here” directives that should not be used, as \LaTeX{} knows better.}
