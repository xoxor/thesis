...
The first records of collective choice problems can be traced back to ancient Greece (ca. 500 BC),
\commentOC{Aren’t there earlier records? Summerians, e.g.?} \commentBN{Not really, all texts I've read identify with the romans the start of CDM because $m>2$, but they mention Greece as we have lots of documents describing their democratic process.} where the word democracy itself has it origins from the greek dēmokratiā: dēmos \textit{people} and kratos \textit{rule}. \commentOC{where it has it origins from…?}\commentBN{ancient Greece ?}
...
\commentOC{This analyzis seems a bit brittle to me.}\commentBN{I mean it's not my opinion, that's what I read around, ancient greeks where not faced with the problems of aggregation so they never analysed them. That's why the letter of Pliny is so relevant.}
...
\commentOC{Really?}\commentBN{Yep}
...
\commentOC{I do not understand “but so it was…”}\commentBN{Typo, it should be "so was".}
...
\commentOC{Nice intro. I want to read the next part.}
