...
This method, in its main idea, has been rediscovered several times over the years. Its proposal can be traced back to \citet{Condorcet1789, Condorcet1793}, whose procedure, translated by \citet[pp. 249-250]{McLean1994}, reads \commentOC{perhaps help the reader relate this description to the previous § by specifying that here $q = 1/2$}: \say{If one candidate has the absolute majority of first votes, he will be elected. If one candidate has the absolute majority of first votes and second votes together, he will be elected. If several candidates obtain this majority, the one with the most votes will be preferred. If one candidate has the absolute majority of the three votes together, he will be elected, and if several candidates obtain this majority, the one with the most votes will be preferred.} 

As \cite{Camps2014} point out, the same method \commentOC{Here you write “the same” (thus including tie-breaking by $q$) but later you write that the tie-breaking one is a variant.} was later analyzed by \cite{Lhuilier1793} and adopted in Geneva. Sometime after that, in the early 20th century, this procedure was promoted by the politician James W. Bucklin and adopted first in the municipality of Grand Junction (Colorado, USA) and then in many other cities of the United States \cite[p. 167]{Barber2000}. In 1986 it was proposed again by \citet{Sertel1986} and \citet{Sertel1999} under the name of Majoritarian Compromise. Later, \citet{Brams2001} generalize\commentOC{d} this concept and introduce a class of \acs{SCR} called $q$\emph{-approval fallback bargaining}, where $q$ is the required quantity of support that can vary from a single voter up to unanimity. Different choices of $q$ lead to different \acp{SCR}. Considering $n$ voters, the choice of $q=1$ corresponds to the plurality rule, $q=\ceil{\frac{n}{2}}$ is the quota used by the Majoritarian Compromise and $q=n$ represents a bargaining procedure called \emph{Fallback bargaining}, which has been further analyzed by \citet{Kibris2007} and \citet{Congar2012}.
...
Given any $ 1 \leq q \leq n$ and $1 \leq d \leq m$, consider the \commentOC{$(q, d)$-}compromise set, i.e. the set of alternatives that are ranked among the top d by at least q voters $\text{CS}^{q}_{d}=\{a \in A \suchthat \#\{r_{\prefi}(a) \leq d, \forall i \in N\} \geq q \}$.
...
\commentOC{That’s clear, but you need to specify that these quantities depend on the profile to be even clearer and formally correct, e.g., $\text{CS}^q_d(P)$.}
...
We can define a revised version of the BK rule by adding an extra step. If multiple alternatives are ranked $d_{q}^{*}$ for at least q bargainers, the \textbf{revised q-approval fallback bargaining} returns the ones with the greatest support: $f_{\text{rBK}}^{q}= \text{CS}^{q}_{d}$ where $d=d_{q}^{*}$ and $q=q_{d}^{*}$. \commentOC{$q$ is already bound. Do you mean $f_{\text{rBK}}^{q}= \text{CS}^{q_{d}^{*}}_{d}$?}
Note that when CS is a singleton, or when all the alternatives in CS are supported by the same amount of voters then $f_{\text{BK}}^{q}=f_{\text{rBK}}^{q}$\commentOC{.}
\commentOC{It would be helpful to observe that this also holds when $q = n$.}
\Cref{ex:BK} shows how different quotas lead to different results.

\begin{example}[\cite{Brams2001}]\commentOC{I’d use citep here (this is a very minor remark but I know that you like those).}
	\label{ex:BK}
	Consider the following profile $P$:
	\begin{center}
		$
...
\commentOC{It is unclear what class that is exactly: all the q approval fbs, all the revised ones as well, some other combination?}
...
Suppose that a slight majority of voters slightly prefer alternative $a$ to alternative $b$, these would appear in the preference rankings as $a \pref b$. Now consider that the remaining slight minority despises $a$, and strongly prefers $b$ to $a$. There is no way for any rule in the framework of ranked preferences to distinguish the difference in intensity between $a \pref b$ in the first group and $b \pref a$ in the second \commentOC{if there are only two alternatives (otherwise the rank difference may be considered as an intensity)}. How to decide what is the most desirable alternative for society?
...
However, we immediately see that we should define lower and upper bounds on the score a voter could assign to avoid problems of exaggeration. \commentOC{This is too informal. It doesn’t really avoid problems of exaggeration, to start with. And “immediately see” is also too strong, it’s certainly not obvious to me for example. I’d simply drop the sentence as a bounded scale will anyway appear more natural to readers than unbounded ones, I suppose; you repeat the point shortly afterwards; and anyway that’s the way it is, you don’t necessarily have to justify this choice up-front.}
...
Here the common language consists of a range of numerical scores, for example from 0 to 100, and the rule works as follows: each voter assigns a score to all or some alternatives, the one with the highest average score is elected. If more than one alternative has the same highest score than \commentOC{then} the tie is broken randomly.

But a whole bunch of new questions arise. What is the right size for this scale? If it is too large it creates \commentOC{may create} manipulation problems because voters may exaggerate their preferences; if it is not large enough it reduces expressiveness. Is it better to use a numerical scale or a more immediate language? Are there really benefits to using ratings instead of rankings? 
...
\commentOC{At this point the reader expects that you consolate her somewhat by writing what she will find in the PhD about this kind of rule, not just what she won’t find.}
...
\subsection*{Majority judgment}
	...
	Considering the axioms aforementioned, \ac{MJ} satisfies anonymity, neutrality, nondictatoriality but it fails manipulability and it does not satisfy the majority criterion. \commentOC{Do these axioms apply here?}
	\begin{example}[\citet{Laslier2018}]
		\label{ex:MJfailsMajority}
		To show that \ac{MJ} is not majoritarian \commentOC{This doesn’t show that MJ is not majoritarian as it doesn’t use its scale.}, consider the following profile formed by three voters $i_1, i_2, i_3$ and two candidates $a, b$. Suppose the voters assign to candidates a grade between 0 and 20.
		\commentOC{I’d restrict the example to the point by Laslier. Then discuss their answer. Then make a second example block with the latter part that is currently here about applying the point to MJ. I think that making explicit the reason for favoring $a$ over $b$ would be helpful.}
		\begin{center}
			$
			\begin{array}{cccc}
	...
	\begin{center}
		...
		$
	\end{center}
	And $a$ would be preferred \commentOC{elected}. 
