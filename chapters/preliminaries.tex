...
Ideally we would not want either of these behaviours \commentOC{I find this a bit too normative.}. In what follows we present two axioms that promote equality among voters, \textit{anonymity}, and among candidates, \textit{neutrality}. 
	...
	We could also think at \commentOC{consider, or think about} a weaker form of anonymity were the voters are not necessarily equal but which does not allow for a dictator\textemdash a specific voter whose preferred choice is always the outcome of the voting rule. 
	...
	\commentOC{The spacing of $(1)$ is wrong: $\prefi$ is used as a function here, not as a relational symbol, but \LaTeX{} is not aware of this.}
	\end{genthm}
...
%	\end{genthm}
	...
	\commentOC{If $n$ is odd, this seems too demanding: equality should suffice, intuitively, if rounding up (I don’t think rounding is useful here).}
	...
	\commentOC{This last sentence should not be part of the definition: it is a remark whose truth follows from the definition.}
	...
	\commentOC{Here, $a$ is a Condorcet winner.}
