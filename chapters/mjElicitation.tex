...
In particular, LaPrimaire.org \citep{LaPrimaire} is a french political initiative whose goal is to select an independent candidate for the french presidential election using \ac{MJ} as voting rule. The association Democratech implemented the platform for the first time in 2016 in view of the 2017 presidential elections. The number of voters who participated in the election was $10676$ during the first round (with $53383$ votes) and $32685$ during the second round (with $163425$ votes). \commentOC{That’s pretty funny, it reads like everybody is cheating like hell, with this number of votes greater than the number of voters.}
...
A \textit{common language} $\triangle = \{ \delta_1, \delta_2, \dots \}$ is a set of strictly ordered grades. It may, or may not, be finite \commentOC{Do you need infiniteness? It seems like a receipt for trouble to me.} and the notation $\delta_1 \geq \delta_2$ indicates that $\delta_1$ is a better or equivalent grade than $\delta_2$. A profile $P : A\times N \rightarrow \triangle$ is a $m$ by $n$ matrix of grades $P \in \triangle^{A \times N}$ \commentOC{$P \in \triangle^{A \times N}$ is redundant}. Given any $N'\subseteq N$, the operator $\rho: \triangle^{N'} \rightarrow \triangle^{\card{N'}}$ defines an ordering function that given a vector of grades $P_j$ returns the vector ordered by decreasing grades.
...
We define $f: \triangle^{N} \rightarrow \triangle$ a function \commentOC{It’s not defined as you don’t say what $f$ is exactly. Rather: We let $f: …$ denote a function…} that assigns to any vector of grades a final grade. Given any $S\subseteq A$, a grading function $f^S: \triangle^{S \times N} \rightarrow \triangle^S$ returns a vector of final grades by applying $f$ to every alternative in $S$.
...
\commentOC{Defining the right, full notations from the start would be more gentle for the reader.} \commentBN{To discuss, I think it is better like this to clearly separate the real definition of MJ with my interpretation of incompleteness} \commentOC{Ok.}
...
The starting knowledge is represented by a matrix $m\times n$ of $\dbar$ grades. Given $k \in \N$, we define with $K_i \subseteq A$ a set of $k$ alternatives that we ask the voter $i\in N$ to judge \commentOC{I think (but am not sure) that “evaluate” is better here.}. 
...
Note that when $k=n$ \commentOC{Do you mean $m$?}, the resulting profile $\Pbar$ corresponds to the complete profile $P$. Let $C(\Pbar) = \{P' \in \triangle^{m \times n} \suchthat \Pbar \subseteq P'\}$ be the set of all completions of $\Pbar$ obtained by substituting all $\dbar$ grades with "defined" ones, note that $P \in C(\Pbar)$.
...
\commentOC{I like your proposal (a reasonable abuse of notation) but, more generally, it appears that this chapter is mostly empirical, thus, it’s not so important to have everything defined formally, so perhaps you can just describe $C(\Pbar)$ informally as you do currently and simply omit its formal definition (which would require introducing $\restr{\Pbar}{\triangle} = \Pbar \cap (A \times N \times \triangle)$ first anyway).}
...
Let $g:\overline{\triangle}^N\rightarrow \bigcup_{N' \subseteq N}\triangle^{N'}$ be a function that given an incomplete vector of grades $q \in \overline{\triangle}^N$, thus $q \subseteq N × \bar{\triangle}$, returns the vector composed only of the "defined" grades $g(q) = \restr{q}{N × \triangle}$ \commentOC{This should be $q \cap (N × \triangle)$, but anyway as above this may be more complexity that what we do with this afterwards justifies, so perhaps omitting what $g$ is formally (especially considering that this union is not straightforward to understand) may be beneficial to the reader.}.
...
\commentOC{The domain of $\fmajbar$ should be the incomplete profiles; $P'$ is not defined; and $j$ does not seem relevant here. Anyway, same suggestion as above: omit the formal definitions.}
